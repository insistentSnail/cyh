...
<script>
	...
	import {getShopInfo,buyCoupon,createCommodityOrder,getCommodityOrderPayInfo} from '@/api/shop/shop.js'
	...
	export default {
		...
		// 	    } else if (res.cancel) {
		// 			console.log('用户点击取消')
		// 			// uni.navigateBack();//onBackPress方法里uni.uni.navigateBack()会失效
		// 	    }
		// 	  }
		// 	})
		...
		methods:{
			...
			toShipAddress(){
				uni.navigateTo({
					url:'../shipAddress/shipAddress?shopId='+this.shopId
				})
			},
			// 跳转修改收获地址
			toUpdateAddress(id){
				uni.redirectTo({
					url:'../myAddress/myAddress?shopId='+this.shopId
				})
			},
			openTypeShow(){
				this.typeShow=true;
