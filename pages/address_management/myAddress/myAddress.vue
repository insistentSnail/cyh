...
<script>
	...
	export default {
		...
		onNavigationBarButtonTap(val) {
			...
			// 添加地址
			uni.redirectTo({
				url:'./addressInfo/addressInfo?shopId='+this.shopId
			})
		},
		// 防止点击返回按钮 反复回退
		onBackPress(event){
			uni.redirectTo({
				url:'../welfareShopOrder/welfareShopOrder?id='+this.shopId
			});
			return true;
		},
		methods:{
			...
			editAddress(id){
				uni.redirectTo({
					url:'./addressInfo/addressInfo?update=1&id='+id+'&shopId='+this.shopId
				})
			},
			// 删除地址
			removeAddress(id){
